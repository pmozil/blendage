...
static void applybounds(Client *c, struct wlr_box *bbox)
{
	...
	c->geom.width = MAX(min.width + (2 * (c->bw + c->gap)), c->geom.width);
	c->geom.height = MAX(min.height + (2 * (c->bw + c->gap)), c->geom.height);
	if (max.width > 0 && !(2 * (c->bw + c->gap) > INT_MAX - max.width)) // Checks for overflow
		c->geom.width = MIN(max.width + (2 * (c->bw + c->gap)), c->geom.width);
	if (max.height > 0 && !(2 * (c->bw + c->gap) > INT_MAX - max.height)) // Checks for overflow
		c->geom.height = MIN(max.height + (2 * c->bw + 2 * c->gap), c->geom.height);

	if (c->geom.x >= bbox->x + bbox->width)
		c->geom.x = bbox->x + bbox->width - c->geom.width;
	if (c->geom.y >= bbox->y + bbox->height)
		c->geom.y = bbox->y + bbox->height - c->geom.height;
	if (c->geom.x + c->geom.width + 2 * (c->bw + c->gap) <= bbox->x)
		c->geom.x = bbox->x;
	if (c->geom.y + c->geom.height + 2 * (c->bw + c->gap) <= bbox->y)
		c->geom.y = bbox->y;
}
...
static void dynamictile(Monitor *m)
{
	...
	wl_list_for_each(c, &clients, link)
		n += (int)(VISIBLEON(c, m) && !c->isfloating && !c->isfullscreen);
